...
def assistant():
    ...
    if request.method == "POST":
        ...
        db.execute("""INSERT INTO conversations(user_id,query,answer,category,language)
                      VALUES(?,?,?,?,?)""",
                   (session["user_id"], query, answer, category, lang))
        db.commit()
...
@app.route("/health")
def health(): return render_template("module.html", title="Health Assistant", icon="🩺", description="General health information, report explanation and questions to discuss with a qualified professional.", bullets=["Explain health terms in simple language","Summarize non-sensitive text reports","Prepare questions for a doctor","Urgent symptoms should be assessed by local emergency services"])

@app.route("/education")
def education(): return render_template("module.html", title="Education Assistant", icon="🎓", description="Learn topics using summaries, quizzes, flashcards and simple explanations.", bullets=["Explain difficult topics","Generate practice questions","Create study summaries","Build a learning roadmap"])

@app.route("/government")
def government(): return render_template("module.html", title="Government Assistant", icon="🏛️", description="Understand government services and schemes using verified knowledge sources.", bullets=["Explain scheme terminology","Create application checklists","Summarize official instructions","Always verify final eligibility on the official portal"])

@app.route("/career")
def career(): return render_template("module.html", title="Career Assistant", icon="💼", description="Turn your skills and goals into practical career roadmaps.", bullets=["Skill-gap analysis","Learning roadmap","Project ideas","Interview preparation"])

@app.route("/agriculture")
def agriculture(): return render_template("module.html", title="Agriculture Assistant", icon="🌾", description="General crop and farming guidance. Add verified local agriculture data for production use.", bullets=["Crop-care guidance","Pest information","Irrigation concepts","Weather-aware planning"])
